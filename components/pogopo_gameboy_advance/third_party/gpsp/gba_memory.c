...
u32 gamepak_buffer_limit = ROM_BUFFER_SIZE;
...
u8 *load_gamepak_page(u32 physical_index)
{
  ...
  const u32 page_start = physical_index * (32 * 1024);
  const u32 bytes_left = page_start < gamepak_file_size
    ? gamepak_file_size - page_start : 0;
  const u32 bytes_to_read = MIN(bytes_left, 32 * 1024);
#ifdef POGOPO_GBA
  const int64_t load_start = esp_timer_get_time();
#endif
  fseek(gamepak_file_large, page_start, SEEK_SET);
  const size_t bytes_read = fread(swap_location, 1, bytes_to_read, gamepak_file_large);
  if(bytes_read < (32 * 1024))
    memset(swap_location + bytes_read, 0xFF, (32 * 1024) - bytes_read);
  gamepak_page_loads++;
#ifdef POGOPO_GBA
  gamepak_page_load_us += (u32)(esp_timer_get_time() - load_start);
#endif
...
void init_gamepak_buffer(void)
{
  ...
  gamepak_page_load_us = 0;
  if (gamepak_buffer_limit == 0 || gamepak_buffer_limit > ROM_BUFFER_SIZE)
    gamepak_buffer_limit = ROM_BUFFER_SIZE;
  while (gamepak_buffer_count < gamepak_buffer_limit)
  {
...
static s32 load_gamepak_raw(const char *name)
{
  ...
  if(gamepak_file_large)
  {
    ...
    gamepak_file_size = (u32)ftell(gamepak_file_large);
    fseek(gamepak_file_large, 0, SEEK_SET);
    gamepak_size = gamepak_file_size;
    ...
    for (i = 0; i < ldblks; i++)
    {
      ...
      const u32 block_start = i * gamepak_buffer_blocksize;
      const u32 bytes_left = block_start < gamepak_file_size
        ? gamepak_file_size - block_start : 0;
      const u32 bytes_to_read = MIN(bytes_left, gamepak_buffer_blocksize);
      const size_t bytes_read = fread(gamepak_buffers[i], 1, bytes_to_read,
                                      gamepak_file_large);
      if(bytes_read < gamepak_buffer_blocksize)
        memset(gamepak_buffers[i] + bytes_read, 0xFF,
               gamepak_buffer_blocksize - bytes_read);
