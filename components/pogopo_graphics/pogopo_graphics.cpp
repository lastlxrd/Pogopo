...
static uint8_t glyph_column(char c, int column) {
    struct Glyph {
        char character;
        uint8_t data[5];
    };
    ...
    static const Glyph glyphs[] = {
        {' ', {0x00,0x00,0x00,0x00,0x00}}, {'!', {0x00,0x00,0x5F,0x00,0x00}}, {'+', {0x08,0x08,0x3E,0x08,0x08}}, {'-', {0x08,0x08,0x08,0x08,0x08}},
    ...
    for (const auto& glyph : glyphs) {
        if (glyph.character == c) {
            return glyph.data[column];
        }
    }
    return (column == 0 || column == 4) ? 0x7F : 0x41;
}

void SharpDisplay::draw_char_unlocked(int x, int y, char c, Color color, int scale) {
    scale = std::max(1, scale);
    for (int column = 0; column < 5; ++column) {
        const uint8_t bits = glyph_column(c, column);
