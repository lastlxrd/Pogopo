...
size_t Storage::listWav(const char* relative_dir, FileEntry* out, size_t capacity) const {
    ...
    char dir_path[192]{};
    if (!relative_dir || !relative_dir[0]) std::snprintf(dir_path, sizeof(dir_path), "%s", config_.mount_point);
    else if (relative_dir[0] == '/') std::snprintf(dir_path, sizeof(dir_path), "%s%s", config_.mount_point, relative_dir);
    else std::snprintf(dir_path, sizeof(dir_path), "%s/%s", config_.mount_point, relative_dir);
    ...
    while (count < capacity) {
        ...
        std::snprintf(item.path, sizeof(item.path), "%s/%s", dir_path, ent->d_name);
        struct stat st{};
