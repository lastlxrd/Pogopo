...
| `.pda` PCM/IMA ADPCM | Supported subset | 8/16-bit mono/stereo PCM and mono IMA are decoded; stereo is downmixed, short effects are cached and long tracks use the music path. |
...
- short sample effects, basic playback rate and a separate music player;
...
- Audio playback supports the package formats and basic controls used by the
  test set, not every Playdate synth/effect/sequence API. Stereo IMA ADPCM and
  file-player loop subranges are not implemented yet.
