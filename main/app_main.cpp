...
void os_task(void*) {
    ...
    if (!pogopo::menu::Assets::valid()) {
        ...
        ESP_LOGI(TAG, "STEP13.4.1 menu assets ready: %u bytes",
                 static_cast<unsigned>(pogopo::menu::Assets::embeddedSize()));
    }
    if (!g_power_outro_animation.valid()) {
        ESP_LOGE(TAG, "Power Outro asset size mismatch: %u bytes",
    ...
    ESP_LOGI(TAG, "STEP13.4.1 fast-start runtime ready after startup");
...
extern "C" void app_main(void) {
    ...
    ESP_LOGI(TAG, "pogopoOS2.0 STEP13.4.1 POGODATE FAST START");
    ...
    start_system_tasks();
    ESP_LOGI(TAG, "STEP13.4.1 system tasks started: startup animation pending");
}
